...
def _load_words(file_name):
    ...
    with codecs.open(file_path, "r", "utf-8") as rfd:
        words_set = set(rfd.read().splitlines()) # 创建一个无序不重复元素集，可进行关系测试，删除重复数据，还可以计算交集、差集、并集等。
    return words_set
...
class NlpUtil(object):
    ...
    for w in user_define_words:
        jieba.add_word(w, freq=1000000) # 用户自定义词汇可以使得分词效率更高，可手动添加自定义词汇

    corpus_dict = None
    tfidf_model = None

    url_pattern = re.compile(r"(https|http)://.+?html") #  complile编译正则表达式模式，返回一个对象。
    digit_pattern = re.compile(r"\d+")
    bracket_pattern = re.compile(r"\[.+?\]")

    not_place_set = set(["淘宝", "上门", "东西", "拜拜", "满意度",
        "新旧", "入口", "莫大", "蓝牙", "英伦", "顺顺利利",
    ...
    def place_recognize(cls, text):
        places = [w for w, flag in pseg.cut(text) if "ns" in flag # pseg.cut词性标注 (ns是地名)
            and len(w) >= 2 
    ...
    def tokenize(cls,
        ...
        try:
            ...
            for w in places:
                text = text.replace(w, "[地址x]") # 把地名全部替换成[地址x]
            text = cls.digit_pattern.sub("[数字x]", text) # 把数字全部替换成[数字x]
            if normalize_url:
                text = cls.url_pattern.sub("URL", text)# 把text中的url全部以URL替换
            tokens = jieba.lcut(text) # 这样直接分词后返回list
            text = " ".join(tokens) 
            for s in cls.bracket_pattern.findall(text):
                text = text.replace(s, s.replace(" ", ""))
            text = text.replace(u"# E - s [数字x]", u"#E-s[数字x]") # u表示含有中文字符组成的字符串。"
            text = text.replace(u"# E - s DIGIT [数字x]", u"#E-s[数字x]")
            text = text.replace(u"< s >", "<s>")
            tokens = text.split() # 以空格为分隔符，包含 \n，分成字符串列表
            tokens_copy = copy.copy(tokens) # 对象拷贝，浅拷贝 引用
            ...
            if filter_alpha:
                tokens = [w for w in tokens if not w.encode("utf-8").isalpha() # isalpha检测字符串是否只由字母组成，是返回true。
                    or w in set(["URL"])]
            if remove_words:
                tokens = [w for w in tokens if w not in cls.remove_words_set]
            # 判断过滤后的token的长度，如果太短了就不过滤
            if len(tokens) < minimum_tokens_num:
                tokens = tokens_copy
            # 将相邻的相同词汇排除
            new_tokens = tokens[:1] # 先取第一个词
            t_len = len(tokens)
            for i in range(1, t_len):
                if tokens[i] != tokens[i - 1]: # 判断是否有相邻词汇相同
...
# msg = "你好123四川+达州"
# print(msg)
# tokenize_msg = NlpUtil.tokenize(msg, filter_punctuations=True, filter_stopwords=True, filter_alpha=True)
# # tfidf = NlpUtil.get_tfidf (["这是？"])
# print("......")
# print(tokenize_msg)
# # print(tfidf)