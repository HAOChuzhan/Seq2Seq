...
invoice_pattern = re.compile(r"发票|报销|增票|专票")

def intent_update(msg, dialog_status):
    if invoice_pattern.search(msg):
        dialog_status.intent = "invoice"
    return dialog_status

def invoice_handle(msg, dialog_status):
    if dialog_status.intent == "invoice":
        answer1 = invoice_judge(msg)
        answer2 = invoice_revise(msg)
        if answer2 or answer1:
            if answer2: return answer2
            if answer1: return answer1
    if not invoice_pattern.search(msg):
        if dialog_status.intent == "invoice":
...
def invoice_judge(sentence):
    ...
    if True:
        ...
        if re.search("谁.*开",sentence):
            return "您好，一般情况下小店自营的商品由本店提供发票；第三方卖家销售的商品由卖家开具并邮寄发票。"
        ...
        if re.search("金额",sentence):
            if re.search("不一致|不一样|不同|不对|不太对",sentence):
                return "您好，发票金额一般是您本次的实际支付金额。使用优惠券、卡、金豆等支付的金额不开发票。"
        ...
        if re.search("查",sentence):
            return "请您按照以下步骤【端我的客户服务发票下载使用】【电脑端我的小店客户服务我的发票发票详情下载使用】可在端打开URL进行发票查询和下载哦"
        ...
        if re.search("帮|要|怎样|怎么|如何|哪.*发票|哪里|开.*票",sentence):
            ...
            if re.search("补开发票",sentence):
                if re.search("图书",sentence):
                    return "您好，您下单时如忘记开具图书发票，小店自营图书商品可在180天内（以订单完成时间开始计算）可使用【补开发票】自助功能申请补开，或联系客服人员为您办理补开发票事宜，快递费用由本店承担。"
            

            if re.search("纸质发票",sentence):return "您好,下单时选择“普通发票（纸质）”自助开取，此发票可用作单位报销凭证，一个订单对应一张或多张发票，不同的物流中心发出的包裹开具不同的发票，发票会随每次包裹一同发出。"
            
            if re.search("电子发票",sentence):return "您好，下单时选择“普通发票（电子）”自助开取，订单完成后，系统会自动开具，用户可登陆我的个人账户，在订单详情页-付款信息页面下载。" 
    ...
    if re.search(sign, sentence):
        ...
        if re.search('想|给|帮|怎么|怎样|如何|(可以|能)修改[嘛吗么]', sentence):
            if re.search("打印",sentence):
                return "已提交申请修改好的发票[数字x]个工作日内开出，端:我的—客户服务—我的发票—发票详情下载即可;端:我的—客户服务—发票服务—发票详情查看即可;微信手端:优选—个人中心—订单详情查看即可载即可;"
            return "亲，您把公司抬头和税号发过来，我这边可以帮您记录修改呢" 
