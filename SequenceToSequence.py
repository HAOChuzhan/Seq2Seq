...
class Seq2Seq(object):
    ...
    def init_placeholder(self):
        ...
        if self.mode == 'train':
            ...
            self.decoder_inputs_train = tf.concat([self.decoder_start_token, self.decoder_inputs], axis=1) # 张量行拼接

    # 在模型定义好后，训练之前会载入预先训练好的词向量
    ...
    def embedding(self):
        ...
        with tf.variable_scope('embedding'):
            ...
            self.encoder_embeddings_placeholder = tf.placeholder(
                tf.float32,
                (self.encoder_vocab_size, self.embedding_dim)# 维度是 vocab_size×300
            )
    ...
    def build_encoder_cell(self, hidden_size, cell_type, layer_size):
        ...
        :return:
        """
        cells = [self.one_cell(hidden_size, cell_type) for _ in range(layer_size)] # 4层 将多个LSTM神经元汇总成一个
    ...
    def build_encoder(self):
        ...
        with tf.variable_scope('encoder'):
            ...
            encoder_inputs_embedded = tf.nn.embedding_lookup(self.encoder_embeddings, self.encoder_inputs) # 此处的encoder_inputs是词id组成的数组，然后去向量中索引
            ...
            if self.bidirection:   # 双向RNN
                ...
                encoder_outputs, encoder_final_state = tf.nn.dynamic_rnn(
                    cell=encoder_cell,
                    inputs=encoder_inputs_embedded, # 是一个tensor 根据id选出来的向量 一般格式为[batch_size, max_time(序列最长长度), embed_size=300]，
                    sequence_length=self.encoder_inputs_length, # 假设输入三句话且各自长度5，10，25，则为[5,10,25]
                    dtype=tf.float32,
    ...
    def build_decoder(self, encoder_outputs, encoder_final_state):
        ...
        with tf.variable_scope("decode"):
            ...
            if self.mode == 'train':
                ...
                max_decoder_length = tf.reduce_max(self.decoder_inputs_length) # 最大的有效长度
                '''
                ...
                '''
                self.loss = tf.contrib.seq2seq.sequence_loss(logits=training_decoder_output.rnn_output, # 当前batch序列最大的有效长度
                                                             targets=self.decoder_inputs, # 本是固定的最大长度，但在训练输入时去切成了有效最大长度
    ...
    def check_feeds(self, encoder_inputs, encoder_inputs_length,
        ...
        # assert 是断言函数，当表达式为真时，继续执行；为假时抛出错误，并输出参数。
